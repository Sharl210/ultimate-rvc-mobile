...
class _PitchDetectionScreenState extends State<PitchDetectionScreen> with WidgetsBindingObserver {
  static const _frequencySmoothingFactor = 0.18;
  ...
  Future<void> _startDetection() async {
    ...
    if (_history.isEmpty) {
      _axisRange = _chartMode == MeasurementChartMode.frequencyHz
          ? const MeasurementAxisRange(min: 0, max: 400)
          : const MeasurementAxisRange(min: -12, max: 12);
    ...
    _subscription = _rvcBridge.pitchStream().listen((event) {
      ...
      while (_frequencyWindow.length > 6) {
      ...
      final nextFrequency = _displayFrequencyHz <= 0
          ? averageFrequency
          : _displayFrequencyHz + (averageFrequency - _displayFrequencyHz) * _frequencySmoothingFactor;
    ...
    _chartTimer = Timer.periodic(_chartTick, (_) {
      ...
      setState(() => _appendHistory(_chartValue(_displayFrequencyHz, _displaySemitoneValue)));
    });
  }
  ...
  void _appendHistory(double value) {
    ...
    final nextRange = buildAxisRange(
      _history,
      previous: _axisRange,
      defaultMin: _chartMode == MeasurementChartMode.frequencyHz ? 0 : -12,
      defaultMax: _chartMode == MeasurementChartMode.frequencyHz ? 400 : 12,
      minimumSpan: _chartMode == MeasurementChartMode.frequencyHz ? 30 : 4,
    );
    _axisRange = _chartMode == MeasurementChartMode.frequencyHz
        ? MeasurementAxisRange(
            min: 0,
            max: nextRange.max < 30 ? 30 : nextRange.max,
          )
        : nextRange;
    _fullscreenPitchRefresh?.call(() {});
  }

  double _chartValue(double frequency, double semitone) {
    return _chartMode == MeasurementChartMode.frequencyHz ? frequency : semitone;
  }
            ...
            FilledButton(
              onPressed: () async {
                ...
                setState(() {
                  ...
                  _displaySemitoneValue = _relativeSemitoneFromReference(_displayFrequencyHz);
                  _history.clear();
                  _axisRange = selectedMode == MeasurementChartMode.frequencyHz
                      ? const MeasurementAxisRange(min: 0, max: 400)
                      : const MeasurementAxisRange(min: -12, max: 12);
                });
                await _saveSettings();
          ...
          child: StatefulBuilder(
            builder: (context, setDialogState) {
              ...
              return MeasurementChartCard(
                ...
                yAxisLabel: _chartMode == MeasurementChartMode.frequencyHz ? 'Hz' : '半音',
