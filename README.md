...
**一个面向 Android 的本地 RVC 移动端项目(仅支持v2版本模型)，支持音频推理、悬浮窗变声器、实时推理（移动端性能受限，权限受限估仅供演示用途）；新增独有特性：降噪优化，音域过滤，噪声过滤**
...
内存占用:主进程: ～200MB，处理进程: 1.1GB～1.8GB（该进程非常驻仅在处理时拉起，用后即焚，一般来讲1.4GB左右,理论上来讲，不管多长的音频，内存也只会保持在这个区间，因为这边做了分片进行，不会一次性看爆内存导致OOM，当然你手机内存本来就小的除外）
